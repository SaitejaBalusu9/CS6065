...
config = {
    'host': 'sankii.mysql.database.azure.com',
    'user': 'sankeerth@sankii',
    'password': 'Sank!1998',
    'database': 'sankii_db',
    ...
    if request.method == 'POST':
        ...
        if hdata.filename == '' or tdata.filename == '' or pdata.filename == '':
            ...
            file_path = os.path.join(
                app.config['UPLOAD_FOLDER'], hdata.filename)
            hdata.save(file_path)
            col_names = ['HSHD_NUM', 'L', 'AGE_RANGE', 'MARITAL', 'INCOME_RANGE',
            ...
            for i, row in csvData.iterrows():
                ...
                value = (row['HSHD_NUM'], row['L'], row['AGE_RANGE'], row['MARITAL'], row['INCOME_RANGE'],
            ...
            # transaction data
            file_path = os.path.join(
                app.config['UPLOAD_FOLDER'], tdata.filename)
            tdata.save(file_path)
            col_names = ['TRANS_ID', 'BASKET_NUM', 'HSHD_NUM', 'PURCHASE_',
            ...
            for i, row in csvData.iterrows():
                ...
                value = (row['TRANS_ID'], row['BASKET_NUM'], row['HSHD_NUM'], row['PURCHASE_'],
                         row['PRODUCT_NUM'], row['SPEND'], row['UNITS'], row['STORE_R'], row['WEEK_NUM'], row['YEAR'])
                cur.execute(query, value)
                conn.commit()
            # Products data
            file_path = os.path.join(
                app.config['UPLOAD_FOLDER'], pdata.filename)
            pdata.save(file_path)
            col_names = ['PRODUCT_NUM', 'DEPARTMENT',
            ...
            for i, row in csvData.iterrows():
                ...
                value = (row['PRODUCT_NUM'], row['DEPARTMENT'], row['COMMODITY'],
